...
//! // use pqcrypto::sign::sphincssha2192fsimple::*;
...
simple_struct!(
    PublicKey,
    ffi::PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_CRYPTO_PUBLICKEYBYTES
);
simple_struct!(
    SecretKey,
    ffi::PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_CRYPTO_SECRETKEYBYTES
);
...
impl DetachedSignature {
    fn new() -> Self {
        DetachedSignature(
            [0u8; ffi::PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_CRYPTO_BYTES],
            0,
        )
    }
}
...
impl primitive::DetachedSignature for DetachedSignature {
    ...
    fn from_bytes(bytes: &[u8]) -> Result<Self> {
        ...
        let expected = ffi::PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_CRYPTO_BYTES;
        if actual > expected {
        ...
        let mut array = [0u8; ffi::PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_CRYPTO_BYTES];
...
pub fn keypair() -> (PublicKey, SecretKey) {
    #[cfg(all(enable_x86_avx2, feature = "avx2"))]
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_keypair!(PQCLEAN_SPHINCSSHA2192FSIMPLE_AVX2_crypto_sign_keypair);
        }
    }
    gen_keypair!(PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_crypto_sign_keypair)
}
...
pub fn sign(msg: &[u8], sk: &SecretKey) -> SignedMessage {
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return gen_signature!(PQCLEAN_SPHINCSSHA2192FSIMPLE_AVX2_crypto_sign, msg, sk);
        }
    }
    gen_signature!(PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_crypto_sign, msg, sk)
}
...
pub fn open(
    ...
    {
        if std::is_x86_feature_detected!("avx2") {
            return open_signed!(PQCLEAN_SPHINCSSHA2192FSIMPLE_AVX2_crypto_sign_open, sm, pk);
        }
    }
    open_signed!(PQCLEAN_SPHINCSSHA2192FSIMPLE_CLEAN_crypto_sign_open, sm, pk)
}
